...
class PaymentOrderLine(models.Model):
    ...
    invoice_id = fields.Many2one("account.invoice", string="Fatura Origem")
...
class PaymentOrder(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string=u"Responsável", required=True
    )
    payment_mode_id = fields.Many2one(
    ...
    data_emissao_cnab = fields.Datetime(
        "Data de Emissão do CNAB", track_visibility="onchange"
    )
